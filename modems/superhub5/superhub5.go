...
func (sh5 *Modem) apiAddress() string {
	...
	if sh5.Protocol == "" {
		...
		if utils.IsPortReachable(sh5.IPAddress, 443) {
			...
			httpClient, err := utils.GetHTTPClientWithCertificates(tlsCertificateBytes)
			if err != nil {
				panic(err)
			}
			sh5.HTTPClient = httpClient
		}
	}
...
func (sh5 *Modem) ParseStats() (utils.ModemStats, error) {
	if sh5.Stats == nil {
		...
		statsData := utils.BoundedParallelGet(queries, 3, sh5.HTTPClient)
